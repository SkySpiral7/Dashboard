...
public class ForecastService {
    ...
    public void getForecast(
            final MainActivity caller, final StringConsumer consumer, double latitude, double longitude) {
                            ...
                            public void onResponse(Call call, Response response) throws IOException {
                                try (ResponseBody responseBody = response.body()) {
                                    ...
                                    String httpResponseAsString =
                                            okResponseConverter.httpResponseAsString(response, responseBodyString);
                                    Log.i(TAG, httpResponseAsString);
                                    if (response.isSuccessful())
                                        consumer.accept(responseBodyString);
